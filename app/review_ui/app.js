  ...
  return fetch(url, options).then(async (response) => {
    ...
    try {
      ...
      if (!response.ok && endpoint.includes("/api/apikey") && response.status === 404) {
        throw new Error("当前后端未启用 API Key 存储接口，请重新启动本地控制台后刷新页面");
      }
      if (!response.ok) {
        throw new Error(`接口不可用（HTTP ${response.status}），请重新启动本地控制台并刷新页面`);
      }
...
function updatePromptMeta(stateLabel) {
  if (promptFileNameEl) {
    promptFileNameEl.textContent = "app/prompts/default_prompt.txt";
  }
...
function buildPipelineTaskCard(taskType, selectedWeek, latestTask) {
  ...
  const assignmentPath = `app/configs/assignments/${weekId}.json`;
  const isPreprocess = taskType === "preprocess";
  const title = isPreprocess ? "作业预处理" : "Agent 多阶段批改";
...
async function saveApiKeyToLocal() {
  ...
  try {
    ...
    setConfigStatus(apiKeyStatusEl, `已保存到 ${data.storePath || "app/configs/env/local.env"}`, "ok");
...
async function resetPromptFile() {
  ...
  try {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`接口不可用（HTTP ${response.status}），请重新启动本地控制台后重试`);
      }
