...
from app.grade_evaluator import (
    evaluate_homework_qwen_vision,
    get_student_result_path,
    sanitize_filename,
    write_failure_result,
)
from app.grading_graph.adapters.batch import candidate_has_provider_error, run_candidate_states, run_student_candidate_from_images
...
from app.project_config import LOCAL_ENV_FILE, load_runtime_config, resolve_api_key
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="auto",
        help="批改引擎；auto 使用 app/configs/agent_pipeline.json 当前正式结果源",
    )
    parser.add_argument(
        "--graph-input-jsonl",
        default=None,
...
def run_candidate_engine(args: argparse.Namespace, runtime_config) -> int:
    ...
    manifest_path = (
        ...
        else Path(__file__).resolve().parent.parent / "evaluation" / "answer_manifests" / runtime_config.week_name / "manifest.json"
    )
