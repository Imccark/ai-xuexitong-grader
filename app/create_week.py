...
def main() -> int:
    ...
    relative_assignment = new_assignment_path.relative_to(REPO_ROOT)
    print(f"[NEXT] 前处理命令：uv run python -m app.run_preprocessing --assignment {relative_assignment}")
    print(f"[NEXT] 批量评分命令：uv run python -m app.run_batch_grading --assignment {relative_assignment} --max-workers 4")
    return 0
