...
from app.create_week import build_assignment_payload, write_json
from app.project_config import (
    LOCAL_ENV_FILE,
    LOCAL_SETTINGS_FILE,
    AssignmentConfig,
...
from app.run_batch_grading import build_result_txt_reference, load_agent_pipeline_config, parse_result_text
from app.grading_graph.review import ReviewConflict, ReviewGateError, ReviewStore
from app.grading_graph.adapters.legacy_result import candidate_to_legacy_projection, legacy_text_to_candidate
from app.grading_graph.schemas import StudentStatus, TeacherDecision
...
class ReviewRepository:
    ...
    def _compiled_manifest_path(self) -> Path:
        path = Path(__file__).resolve().parent.parent / "evaluation" / "answer_manifests" / self.assignment_config.assignment_id / "manifest.json"
        if not path.is_file():
    ...
    def _rerun_provider(self) -> Any:
        ...
        from app.run_batch_grading import load_agent_pipeline_config
    ...
    def rerun_review_question(self, student_id: str, question_id: str, payload: dict) -> dict:
        ...
        from app.grading_graph.adapters.rerun import (
            persist_targeted_rerun_audit,
            run_targeted_question_rerun,
        )
        from app.grading_graph.schemas import Budget
...
def create_week_resources(week_name: str, force: bool = False) -> dict:
    from app.project_config import DEFAULT_ANSWER_KEY_FILENAME, REPO_ROOT
...
def _assignment_path_from_week_id(week_id: str) -> Path:
    assignment_path = _app_root() / "configs" / "assignments" / f"{week_id}.json"
...
def create_handler():
    class ReviewRequestHandler(BaseHTTPRequestHandler):
        ...
        def _read_prompt_file(self) -> dict:
            prompt_path = (_app_root() / "prompts" / "default_prompt.txt").resolve()
            if not prompt_path.is_file():
                self.send_error(HTTPStatus.NOT_FOUND, "prompt file not found")
                return
            content = prompt_path.read_text(encoding="utf-8")
            self.send_json({"content": content, "path": "app/prompts/default_prompt.txt"})
        ...
        def _write_prompt_file(self, raw_body: bytes) -> None:
            ...
            self.send_json({"ok": True, "path": "app/prompts/default_prompt.txt"})

        def _reset_prompt_file(self) -> None:
            prompt_path = (_app_root() / "prompts" / "default_prompt.txt").resolve()
            prompt_default_path = (_app_root() / "prompts" / "default_prompt.default.txt").resolve()
            ...
            prompt_path.write_text(content, encoding="utf-8")
            self.send_json({"ok": True, "content": content, "path": "app/prompts/default_prompt.txt"})

        def _read_subjects_json(self) -> dict:
            subjects_path = (_app_root() / "configs" / "subjects.json").resolve()
